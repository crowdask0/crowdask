    ...
    */
{
    ...
    if (@$options['voteview']) {
        ...
        */
        ...
        if($isanswer)
        {
            list($parent_q,$temp) = qa_db_select_with_pending(qa_db_post_parent_q_selectspec($postid),null);
            if(isset($parent_q['closedvotesbyid']))
                $fields['vote_state']='disabled';

            //it is better to clear the cache
            foreach(array_keys($parent_q) as $key)
            {
                unset($parent_q[$key]);
            }
            unset($parent_q);
    ...
    */
{
    ...
    if (!isset($parentid))
    {
    	...
    	$navigation['non-categorized']=array(
    			'url' => qa_path_html('questions/non-categorized'),
    			'label' => 'Non Categorized',
    			'note' => $showqcount ? ('(...)') : null,
    			'selected' => !count($selecteds),
    			'categoryid' => null,
    	);
    }
